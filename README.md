...
## Highlights and Red Flags panels
- UI includes two inline actions per analysis card:
  - `✅ 5 Things to Know`
  - `🚩 Red Flags`
- Panels are inline expand/collapse (not modal), and only one panel is open per card at a time.
- First click generates and caches JSON in DB (`highlights_json`, `redflags_json`); later clicks serve cached data.
- Backend APIs:
  - `GET /api/scan/:id/highlights`
  - `GET /api/scan/:id/redflags`
